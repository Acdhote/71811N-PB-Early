...
void default_constants() {
  ...
  chassis.pid_odom_angular_constants_set(11.5, 0.1, 32, 5);    // Angular control for odom motions
  chassis.pid_odom_boomerang_constants_set(5.2, 0.0, 40.5);  // Angular control for boomerang motions 

  // Exit conditions
  chassis.pid_turn_exit_condition_set(90_ms, 1_deg, 250_ms, 2_deg, 500_ms, 500_ms);
  ...
  chassis.odom_boomerang_dlead_set(0.625);     // Reduced aggressiveness for smoother end approach
...
void odom_test() {
  ...
  chassis.pid_odom_set({{-4.5, 53.5, -45}, fwd, 80});
  chassis.pid_wait();

  pros::delay(500);

  chassis.pid_odom_set({{4, 44}, rev, 80});
  chassis.pid_wait();

  chassis.pid_turn_set(128, 80);
  chassis.pid_wait();

  chassis.pid_odom_set({{25,7, 180}, fwd, 80});
  chassis.pid_wait();

  chassis.pid_odom_set({{25, 2.5, 180}, fwd, 80});
  chassis.pid_wait();
